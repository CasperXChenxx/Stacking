...
## E) Preprocessing options (popular)
...
drop_linear_dependencies <- FALSE  # (kept for compatibility; not used now)

## F) Tree path thinning
...
# ============================================================
# 0) Load + preprocessing for superconductivity train.csv
#    Dataset is already numeric; target column is typically "critical_temp"
# ============================================================

raw_data <- read.csv(data_file, stringsAsFactors = FALSE, check.names = FALSE)

# Make column names syntactic but stable (avoids later name issues)
colnames(raw_data) <- make.names(colnames(raw_data), unique = TRUE)

# Identify target column
target_candidates <- c("critical_temp", "critical.temp", "criticaltemp",
                       "critical_temperature", "target", "y")
target_col <- target_candidates[target_candidates %in% names(raw_data)][1]

if (is.na(target_col) || length(target_col) == 0) {
  target_col <- names(raw_data)[ncol(raw_data)]
  warning(sprintf("Could not find 'critical_temp' by name; using last column as target: %s", target_col))
} else {
  cat(sprintf("Using target column: %s\n\n", target_col))
}

# Force numeric (safe even if already numeric)
...
y <- log(raw_data[[target_col]])
X_df <- raw_data[, setdiff(names(raw_data), target_col), drop = FALSE]

# Missing-value imputation (median), if needed
if (impute_missing) {
...
# Optional standardize (NOT needed if you keep whitening later)
...
# ============================================================
# 5) Tuning parameters (same as your code)
# ============================================================
